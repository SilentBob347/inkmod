...
constexpr int kTitleFontId = UI_12_FONT_ID;     // Основной заголовок: 12px
constexpr int kSubtitleFontId = SMALL_FONT_ID;  // Подзаголовок: 8px
constexpr int kGuideFontId = SMALL_FONT_ID;     // Шрифт подсказок
...
void RoundedRaffTheme::drawHeader(const GfxRenderer& renderer, Rect rect, const char* title, const char* subtitle,
  ...
  (void)subtitle;
  // Clear the whole header strip before drawing anything into it — otherwise
  // stale clock/title/battery pixels from the previous minute are never
  // erased and the clock text appears to "slide"/smear on e-ink partial
  // refreshes. LyraTheme and MinimalTheme both do this same clear.
  ...
  drawTopStatusBarClock(renderer, rect.y, nullptr, readerContext,
                        readerContext ? 0 : homeHeaderClockTextYOffset(renderer),
                        false, readerContext ? -1 : batteryGroupLeftX - clockBatteryGap);
}
...
void RoundedRaffTheme::drawTabBar(const GfxRenderer& renderer, Rect rect, const std::vector<TabInfo>& tabs,
  ...
  for (int i = 0; i < tabCount; i++) {
    ...
    // Автоматическое усечение имени вкладки, если русский текст слишком длинный
    const int maxTabLabelWidth = tabWidth - 4;
    auto truncatedTabLabel = renderer.truncatedText(kTitleFontId, tab.label, maxTabLabelWidth, EpdFontFamily::BOLD);
    const int textWidth = renderer.getTextWidth(kTitleFontId, truncatedTabLabel.c_str(), EpdFontFamily::BOLD);
    const int textX = tabX + (tabWidth - textWidth) / 2;
    const int textY = tabY + (tabHeight - renderer.getLineHeight(kTitleFontId)) / 2;
    renderer.drawText(kTitleFontId, textX, textY, truncatedTabLabel.c_str(), !(tab.selected), EpdFontFamily::BOLD);
  }
...
void RoundedRaffTheme::drawRecentBookCover(GfxRenderer& renderer, Rect rect, const std::vector<RecentBook>& recentBooks,
  ...
  if (hasContinueReading) {
    ...
    constexpr int noBookTextMargin = 12;
    auto noBookLabel = renderer.truncatedText(kTitleFontId, tr(STR_NO_OPEN_BOOK), tileWidth - noBookTextMargin * 2);
...
void RoundedRaffTheme::drawButtonMenu(GfxRenderer& renderer, Rect rect, int buttonCount, int selectedIndex,
  ...
  const int rowHeight = renderer.getLineHeight(kTitleFontId) + 15;
...
void RoundedRaffTheme::drawList(const GfxRenderer& renderer, Rect rect, int itemCount, int selectedIndex,
  ...
  for (int i = pageStartIndex; i < itemCount && i < pageStartIndex + pageItems; i++) {
    ...
    // ПРАВКА: Выравнивание ширины правого значения (rowValue) с жестким усечением
    if (rowValue) {
      std::string valueText = rowValue(i);
      if (!valueText.empty()) {
        const int maxValueWidth = std::min(140, std::max(0, rowWidth - kInteractiveInsetX * 2 - kMinValueGap - kMinTitleWidth));
        if (maxValueWidth > 0) {
...
void RoundedRaffTheme::drawButtonHints(GfxRenderer& renderer, const char* btn1, const char* btn2, const char* btn3,
  ...
  // Size each pill to its own label content instead of always splitting the
  // screen 50/50 - a "Back" label doesn't need the same width as "Settings".
  constexpr int innerEdgePadding = 8;
  constexpr int labelGap = 12;        // Gap between the two labels inside one pill
  constexpr int minGroupWidth = 130;  // Keep pills from looking cramped when labels are short
  const int maxGroupWidth = std::max(minGroupWidth, (pageWidth - sidePadding * 2 - groupGap) / 2);
  const int maxLabelWidth = maxGroupWidth - innerEdgePadding * 2 - labelGap;

  const std::string backLabel = backDisabled ? "" : renderer.truncatedText(kGuideFontId, leftOuterLabel, maxLabelWidth, EpdFontFamily::REGULAR);
  const std::string selectText = (leftInnerLabel && leftInnerLabel[0] != '\0') ? renderer.truncatedText(kGuideFontId, leftInnerLabel, maxLabelWidth, EpdFontFamily::REGULAR) : "";
  const std::string upText = (rightInnerLabel && rightInnerLabel[0] != '\0') ? renderer.truncatedText(kGuideFontId, rightInnerLabel, maxLabelWidth, EpdFontFamily::REGULAR) : "";
  const std::string downText = (rightOuterLabel && rightOuterLabel[0] != '\0') ? renderer.truncatedText(kGuideFontId, rightOuterLabel, maxLabelWidth, EpdFontFamily::REGULAR) : "";
  ...
  const int leftGroupWidth =
      std::clamp(backWidth + selectWidth + labelGap + innerEdgePadding * 2, minGroupWidth, maxGroupWidth);
  const int rightGroupWidth =
      std::clamp(upWidth + downWidth + labelGap + innerEdgePadding * 2, minGroupWidth, maxGroupWidth);

  // Center the whole back/select + up/down block as a unit, instead of pinning
  // each pill to the screen edges.
  const int totalWidth = leftGroupWidth + groupGap + rightGroupWidth;
  const int leftGroupX = std::max(sidePadding, (pageWidth - totalWidth) / 2);
  const int rightGroupX = leftGroupX + leftGroupWidth + groupGap;

  renderer.fillRect(leftGroupX, outlineY, leftGroupWidth, hintHeight, false);
  renderer.fillRect(rightGroupX, outlineY, rightGroupWidth, hintHeight, false);

  renderer.drawRoundedRect(leftGroupX, outlineY, leftGroupWidth, hintHeight, 2, kBottomRadius, true);

  const int backX = leftGroupX + innerEdgePadding;
  const int selectX = leftGroupX + leftGroupWidth - innerEdgePadding - selectWidth;
  const int upX = rightGroupX + innerEdgePadding;
  const int downX = rightGroupX + rightGroupWidth - innerEdgePadding - downWidth;

  renderer.drawRoundedRect(rightGroupX, outlineY, rightGroupWidth, hintHeight, 2, kBottomRadius, true);
