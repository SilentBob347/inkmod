...
void FrontlightPanelActivity::onEnter() {
  ...
  if (SETTINGS.touchReaderControls != InkMODSettings::TOUCH_READER_OFF) {
...
void FrontlightPanelActivity::runTile(const int idx) {
  switch (idx) {
    ...
    case 3:  // Touch reader controls (for reading with the palm on the glass)
      ...
      if (SETTINGS.touchReaderControls != InkMODSettings::TOUCH_READER_OFF) {
        touchModeRestore = SETTINGS.touchReaderControls;
        SETTINGS.touchReaderControls = InkMODSettings::TOUCH_READER_OFF;
      } else {
...
void FrontlightPanelActivity::close() {
  // CrossPoint behaviour: the sheet is an overlay. Closing it only pops the
  // activity; the screen underneath is restored by its normal FAST repaint.
  // The UC8279 X4 Pro driver owns the differential old/new-plane bookkeeping,
  // so forcing a FULL refresh here only adds an unnecessary visible flash.
  finish();
}
...
void FrontlightPanelActivity::addSliderRow(UiScreen& screen, const char* label, const uint8_t value,
  ...
  rowProps.buttonRadius = 18;
  rowProps.capsuleRadius = 28;
  if (showToggle) {
...
void FrontlightPanelActivity::buildPanelScreen(UiScreen& screen) {
  ...
  // Phone-like hanging sheet: only the lower corners are rounded by fui::sheet
  // when anchored to the top. The grabber stays centered on the hanging edge.
  sheetProps.radius = 24;
  ...
  if (mappedInput.hasTouch()) {
    ...
    const bool touchOn = SETTINGS.touchReaderControls != InkMODSettings::TOUCH_READER_OFF;
    char touchLabel[48];
