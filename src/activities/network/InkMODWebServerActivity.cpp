...
constexpr const char* AP_SSID = "InkMOD-Reader";
constexpr const char* AP_PASSWORD = nullptr;  // Open network for ease of use
constexpr const char* AP_HOSTNAME = "inkmod";
...
void InkMODWebServerActivity::onEnter() {
  Activity::onEnter();
...
void InkMODWebServerActivity::onExit() {
  Activity::onExit();
...
void InkMODWebServerActivity::onNetworkModeSelected(const NetworkMode mode) {
...
void InkMODWebServerActivity::onWifiSelectionComplete(const bool connected) {
...
void InkMODWebServerActivity::startAccessPoint() {
...
void InkMODWebServerActivity::startWebServer() {
  LOG_DBG("WEBACT", "Starting web server...");

  // Create the web server instance
  webServer.reset(new InkMODWebServer());
  webServer->begin();
...
void InkMODWebServerActivity::exitToOrigin() {
...
void InkMODWebServerActivity::stopWebServer() {
...
void InkMODWebServerActivity::loop() {
...
void InkMODWebServerActivity::render(RenderLock&&) {
...
void InkMODWebServerActivity::renderServerRunning() const {
  const auto& metrics = UITheme::getInstance().getMetrics();
  const auto pageWidth = renderer.getScreenWidth();
  ...
  if (isApMode) {
    // AP mode display
    // STR_CONNECT_WIFI_HINT is not guaranteed to fit on one line in every
    // language (e.g. Russian is noticeably longer than English), so wrap it
    // instead of drawing a single fixed-position line that can run off-screen.
    const int hintMaxWidth = pageWidth - 2 * metrics.contentSidePadding;
    const auto hintLines =
        renderer.wrappedText(UI_10_FONT_ID, tr(STR_CONNECT_WIFI_HINT), hintMaxWidth, 2, EpdFontFamily::BOLD);
...
void InkMODWebServerActivity::renderWifiIndicator(int subHeaderTop) const {
