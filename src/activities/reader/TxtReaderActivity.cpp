...
void TxtReaderActivity::loop() {
  ...
  if (SETTINGS.sideButtonLongPress == InkMODSettings::SIDE_LONG_PRESS::SIDE_LONG_ORIENTATION_CHANGE) {
  ...
  if (SETTINGS.longPressButtonBehavior == InkMODSettings::ORIENTATION_CHANGE || frontLongPressChangesFont) {
...
bool TxtReaderActivity::executePowerButtonAction() {
  ...
  if (mappedInput.wasReleased(MappedInputManager::Button::Power) &&
      mappedInput.getHeldTime() < SETTINGS.getPowerButtonLongPressDuration()) {
    return executeAction(static_cast<InkMODSettings::SHORT_PWRBTN>(SETTINGS.shortPwrBtn));
  }

  const auto longPowerAction = static_cast<InkMODSettings::SHORT_PWRBTN>(SETTINGS.longPwrBtn);
  if (longPowerAction == InkMODSettings::SHORT_PWRBTN::PAGE_TURN || !consumeLongPowerButtonHold()) {
    return false;
  }
...
bool TxtReaderActivity::executeLongPressBackAction() {
  switch (static_cast<InkMODSettings::LONG_PRESS_MENU_ACTION>(SETTINGS.longPressBackAction)) {
    ...
    case InkMODSettings::LONG_PRESS_MENU_ACTION::LONG_MENU_CREATE_HOTSPOT:
      activityManager.goToHotspotFileTransfer(txt ? txt->getPath() : "");
      return true;
    case InkMODSettings::LONG_PRESS_MENU_ACTION::LONG_MENU_TOGGLE_DARK_MODE:
      toggleDarkMode();
      return true;
...
void TxtReaderActivity::renderPage() {
  ...
  auto renderLines = [&]() {
    ...
    for (const auto& line : currentPageLines) {
      if (!line.empty()) {
        ...
        if (lineIsRtl && (effectiveAlignment == InkMODSettings::LEFT_ALIGN ||
                          effectiveAlignment == InkMODSettings::JUSTIFIED)) {
          effectiveAlignment = InkMODSettings::RIGHT_ALIGN;
        }
        const int textWidth = renderer.getTextAdvanceX(cachedFontId, line.c_str(), EpdFontFamily::REGULAR);

        // Apply text alignment
        switch (effectiveAlignment) {
          case InkMODSettings::LEFT_ALIGN:
          default:
            // x already set to left margin
            break;
          case InkMODSettings::CENTER_ALIGN: {
            x = cachedOrientedMarginLeft + (contentWidth - textWidth) / 2;
            break;
          }
          case InkMODSettings::RIGHT_ALIGN: {
            x = cachedOrientedMarginLeft + contentWidth - textWidth;
            break;
          }
          case InkMODSettings::JUSTIFIED:
  ...
  renderLines();
  renderStatusBar();
  if (ReaderUtils::shouldShowTopClockStatusBar()) {
...
void TxtReaderActivity::renderStatusBar() const {
  ...
  std::string title;
  if (SETTINGS.statusBarTitle != InkMODSettings::STATUS_BAR_TITLE::HIDE_TITLE) {
    title = txt->getTitle();
  }
...
bool TxtReaderActivity::drawCurrentPageToBuffer(const std::string& filePath, GfxRenderer& renderer) {
  ...
  switch (SETTINGS.orientation) {
    case InkMODSettings::ORIENTATION::PORTRAIT:
      renderer.setOrientation(GfxRenderer::Orientation::Portrait);
      break;
    case InkMODSettings::ORIENTATION::LANDSCAPE_CW:
      renderer.setOrientation(GfxRenderer::Orientation::LandscapeClockwise);
      break;
    case InkMODSettings::ORIENTATION::INVERTED:
      renderer.setOrientation(GfxRenderer::Orientation::PortraitInverted);
      break;
    case InkMODSettings::ORIENTATION::LANDSCAPE_CCW:
