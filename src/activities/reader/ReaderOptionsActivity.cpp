...
void ReaderOptionsActivity::openLineHeightPicker() {
  startActivityForResult(
      std::make_unique<IntervalSelectionActivity>(
          ...
          StrId::STR_PERCENT_STEP_HINT, SETTINGS.lineHeightPercent, InkMODSettings::MIN_LINE_HEIGHT_PERCENT,
          InkMODSettings::MAX_LINE_HEIGHT_PERCENT, 1, 10, StrId::STR_NONE_OPT, /*readerActivity=*/true,
...
void ReaderOptionsActivity::render(RenderLock&&) {
  ...
  const bool selectedLineHeight = selectedIndex >= 0 && selectedIndex < settingsCount &&
                                  (*currentSettings)[selectedIndex].valuePtr == &InkMODSettings::lineHeightPercent;
  const bool selectedScreenMargin = selectedIndex >= 0 && selectedIndex < settingsCount &&
                                    (*currentSettings)[selectedIndex].valuePtr == &InkMODSettings::screenMargin;
