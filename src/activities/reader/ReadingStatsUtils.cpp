...
// Explicit switch instead of "StrId::STR_CAL_MONTH_1 + offset": gen_i18n.py does not
// guarantee STR_CAL_MONTH_1..12 end up as consecutive enum values (see the same
// pattern/warning in SleepActivity.cpp's calendarMonthName()).
const char* localizedMonthName(const uint8_t month) {
  switch (month) {
    case 1:
...
// Reading stats cells are narrow, so we show an abbreviated month (matching the
// old "Jan"/"Feb" style) instead of the full localized name. Takes the first 3
// Unicode codepoints rather than the first 3 bytes so multi-byte UTF-8 scripts
// (Cyrillic, etc.) aren't cut mid-character - this happens to match the
// conventional 3-letter abbreviation in English, Russian, and Ukrainian alike
// (e.g. "Январь" -> "Янв", "Січень" -> "Січ").
std::string abbreviatedMonthName(const uint8_t month) {
  const char* full = localizedMonthName(month);
  std::string result;
  int codepoints = 0;
  for (const char* p = full; *p != '\0'; ++p) {
    // UTF-8 continuation bytes have the form 10xxxxxx. Only stop right before a
    // 4th codepoint would start, so a multi-byte 3rd codepoint's continuation
    // bytes are never cut off mid-character.
    const bool isStartByte = (static_cast<unsigned char>(*p) & 0xC0) != 0x80;
    if (isStartByte) {
      if (codepoints == 3) break;
      codepoints++;
    }
    result.push_back(*p);
  }
...
void formatReadingStatsShortDate(const ReadingStatsDate& date, char* buf, const size_t len) {
  ...
  snprintf(buf, len, "%s %u", abbreviatedMonthName(date.month).c_str(), static_cast<unsigned>(date.day));
}
...
void formatReadingStatsMonthToken(const ReadingStatsDate& date, char* buf, const size_t len) {
  ...
  snprintf(buf, len, "%s", abbreviatedMonthName(date.month).c_str());
}
