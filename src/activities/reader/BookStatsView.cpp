...
void drawDurationRows(GfxRenderer& renderer, const int x, const int y, const int w, const int h,
  ...
  // Text-only rows use two fixed columns: labels on the left and durations
  // starting at one X coordinate. This keeps minute values aligned even when
  // localized weekday names have different lengths.
  int widestLabel = 0;
  for (size_t i = 0; i < N; ++i) {
    widestLabel =
        std::max(widestLabel, renderer.getTextWidth(layout.chartLabelFontId, I18N.get(labels[i])));
  }
  const int durationX = x + labelLeftPadding + widestLabel + 14;
  const int rightEdge = x + w - labelLeftPadding;
  ...
  for (size_t i = 0; i < N; ++i) {
    ...
    const std::string visibleDuration =
        renderer.truncatedText(layout.chartLabelFontId, duration, std::max(1, rightEdge - durationX));
    renderer.drawText(layout.chartLabelFontId, durationX, labelY, visibleDuration.c_str());
  }
}
