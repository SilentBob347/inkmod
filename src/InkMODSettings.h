...
class InkMODSettings {
 private:
  ...
  InkMODSettings() = default;

  // Static instance
  static InkMODSettings instance;

 public:
  // Delete copy constructor and assignment
  InkMODSettings(const InkMODSettings&) = delete;
  InkMODSettings& operator=(const InkMODSettings&) = delete;
  ...
  enum SLEEP_SCREEN_MODE {
    ...
    DASHBOARD_SLEEP = 11,
    CALENDAR_SLEEP = 12,
    CALENDAR_SLEEP_INVERTED = 13,
    CALENDAR_SLEEP_LANDSCAPE = 14,
    CALENDAR_SLEEP_LANDSCAPE_INVERTED = 15,
    SLEEP_SCREEN_MODE_COUNT
  };
  enum SLEEP_SCREEN_COVER_MODE { FIT = 0, CROP = 1, BLACK_BACKGROUND = 2, SLEEP_SCREEN_COVER_MODE_COUNT };
  ...
  // System-wide interface text size (home screen, menus, settings - NOT the
  // reader font, which has its own separate size control). OFF keeps the
  // firmware's normal built-in sizes; LARGE reuses the next-larger built-in
  // Inter size for each UI font role, so no extra font assets are needed.
  // Raw values are persisted in settings; keep existing values stable.
  enum UI_TEXT_SIZE { UI_TEXT_SIZE_OFF = 0, UI_TEXT_SIZE_LARGE = 1, UI_TEXT_SIZE_COUNT = 2 };
  ...
  enum LONG_PRESS_MENU_ACTION {
    ...
    LONG_MENU_CREATE_CLIPPING = 21,
    LONG_MENU_QUICK_SYNC = 22,
    LONG_MENU_QUICK_SYNC_LEGACY_ALT = 23,
  ...
  enum QUICK_RESUME_SLEEP_SCREEN {
    ...
    TIMEOUT_SLEEP_OVERLAY = 2,
    TIMEOUT_SLEEP_CUSTOM = 3,
    TIMEOUT_SLEEP_COVER = 4,
  ...
  // X4-only: fully disables the clock - no display anywhere, no silent boot-time
  // WiFi/NTP sync, and the Calendar sleep screen is removed from the picker.
  // X3 has a battery-backed RTC and is unaffected regardless of this value;
  // callers should gate showing this setting on !gpio.deviceIsX3().
  uint8_t clockDisabled = 0;
  ...
  // X4 only: UTC epoch seconds from the last successful NTP sync. Persisted so that on a
  // boot where no WiFi network is reachable, the software clock can be seeded with this
  // last-known value (see HalClock::seedFallbackTime()) instead of showing no time at all -
  ...
  // System-wide interface text size (accessibility) - see UI_TEXT_SIZE.
  uint8_t uiTextSize = UI_TEXT_SIZE_OFF;
  // Recent Books screen layout
  uint8_t recentBooksView = RECENT_BOOKS_LIST;
  // Controls only the Search shortcut/icon on Home; search functionality
  // itself remains available and no book data is affected.
  uint8_t showHomeSearch = 1;
  // Sunlight fading compensation. Keep the panel's analogue power rails off
  // between updates so the displayed image remains stable in bright sunlight.
  uint8_t fadingFix = 1;
  ...
  // Bookerly is the factory default; the supplied font pack installs it at
  // /.fonts/Bookerly on the SD card.
  char sdFontFamilyName[64] = "Bookerly";
  ...
  static InkMODSettings& getInstance() { return instance; }
  ...
  uint16_t getPowerButtonWakeDuration() const {
    return (shortPwrBtn == InkMODSettings::SHORT_PWRBTN::SLEEP) ? POWER_BUTTON_WAKE_SHORT_MS
                                                                    : POWER_BUTTON_WAKE_LONG_MS;
  }

  bool shouldShowClockInReader() const { return !clockDisabled && hideClock == HIDE_CLOCK_NEVER; }
  bool shouldShowClockOutsideReader() const {
    return !clockDisabled && (hideClock == HIDE_CLOCK_NEVER || hideClock == HIDE_CLOCK_IN_READER);
  }
  ...
  static void validateFrontButtonMapping(InkMODSettings& settings);
  static void validateReaderFrontButtonMapping(InkMODSettings& settings);
...
#define SETTINGS InkMODSettings::getInstance()
