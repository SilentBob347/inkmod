...
- Firmware update checks now use the `alpzoloto-sudo/rus_mod` GitHub Releases feed.
...
### Changed
- inkMOD settings now save to `/.inkmod/inkmod-settings.json`, with a one-time fallback migration from `/.inkmod/settings.json`, so switching between firmware builds is less likely to reset preferences.
...
- Nearby Reading Stats sync with separate totals for this device and all synced inkMOD readers.
...
### Fixed
- Lyra Carousel is now included by activating the build flag `DINKMOD_ENABLE_LYRA_CAROUSEL=1`
---
## [v1.2.11] - 2026-05-14
