...
def _is_rc_artifact_build(env):
    flag = _get_project_option(env, 'custom_rc_artifacts') or os.environ.get('INKMOD_RC_ARTIFACTS')
    return str(flag).strip().lower() in {'1', 'true', 'yes', 'on'}


def _get_release_version(env):
    version = _get_project_option(env, 'custom_release_version') or os.environ.get('INKMOD_RELEASE_VERSION')
    if not version:
        return None
