...
class GfxRenderer {
 ...
 private:
  ...
  // One-shot refresh promotion used by the X4 Pro control-center refresh tile.
 ...
 public:
  ...
  /// Like insertFont(), but overwrites an existing registration instead of
  /// refusing it. Used to swap which compiled font size backs a fixed UI
  /// font ID (e.g. SMALL_FONT_ID) at runtime, such as for a "large text"
  /// accessibility setting - the font ID stays the same everywhere it's
  /// used, only what it points to changes.
