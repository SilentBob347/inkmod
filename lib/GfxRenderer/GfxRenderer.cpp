...
void GfxRenderer::replaceFont(const int fontId, EpdFontFamily font) { fontMap.insert_or_assign(fontId, font); }
...
void GfxRenderer::displayBuffer(const HalDisplay::RefreshMode refreshMode, const bool turnOffScreen) const {
  ...
  HalDisplay::RefreshMode effective = refreshMode;
  if (promotedRefreshPending_) {
    effective = promotedRefresh_;
    promotedRefreshPending_ = false;
  }
  display.displayBuffer(effective, fadingFix || turnOffScreen);
}
...
int GfxRenderer::getTextAdvanceX(const int fontId, const char* text, EpdFontFamily::Style style) const {
  ...
  while ((cp = utf8NextCodepoint(reinterpret_cast<const uint8_t**>(&text)))) {
    ...
    if ((style & (EpdFontFamily::SUP | EpdFontFamily::SUB | EpdFontFamily::SMALL_CAPS)) != 0) {
