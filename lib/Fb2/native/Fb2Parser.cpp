...
bool Fb2Parser::scan(IByteReader& reader, Fb2ScanResult& out) {
    out = Fb2ScanResult{};

    const uint32_t startPos = reader.position();
    uint32_t sectionCount = 0, binaryCount = 0;
    countTwoTagOpeners(reader, "<section", sectionCount, "<binary", binaryCount);
    out.sections.reserve(sectionCount);
    out.binaries.reserve(binaryCount);
    reader.seek(startPos);

    // 4096 matches the old expat-based converter's read chunk size. The
    // default 512-byte buffer means 8x more IByteReader::read() calls for
    // the same file, and on this firmware every SD read takes a FreeRTOS
    // mutex + SPI bus lock (HalStorage::StorageLock) - that per-call
    // overhead, multiplied by thousands of extra reads on a multi-MB book,
    // is what actually made large books take minutes to open.
