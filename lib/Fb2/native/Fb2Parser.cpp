...
bool Fb2Parser::scan(IByteReader& reader, Fb2ScanResult& out, size_t xmlBufferSize) {
    ...
    for (;;) {
        ...
        if (tok == Fb2Token::StartTag || tok == Fb2Token::SelfClosing) {
            ...
            else if (currentBodyIndex >= 0 && !sectionStack.empty()) out.sections[sectionStack.back()].approxTextBytes += static_cast<uint32_t>(xml.textSize());
        }
        ...
        if (tok == Fb2Token::EndTag) {
            ...
            else if (name == "body") { currentBodyIndex = -1; sectionStack.clear(); }
            else if (name == "section" && !sectionStack.empty()) sectionStack.pop_back();
            else if (name == "title") {
